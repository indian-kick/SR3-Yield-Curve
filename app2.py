...
@st.cache_data
def load_data():
    df = pd.read_csv("SR3_rates_all.csv")
    df['Date'] = pd.to_datetime(df['Date'])
    df = df.sort_values('Date')
    df['DateOnly'] = df['Date'].dt.date

    # Ensure numeric type for all contracts
    maturities = [f'SRAcm{i}' for i in range(1, 17)]
    for m in maturities:
...
def plot_with_bollinger(df, series, label):
    ...
    fig.add_trace(go.Scatter(x=df['Date'], y=lower, mode='lines', name='Lower Band', line=dict(width=0),
...
st.title("Interactive SR3 Futures Rates Visualization")
...
# === Tabs ===
main_tab, outright_tab, spread_tab, fly_tab, condor_tab = st.tabs(["Rates & Curve", "Outrights", "Spreads", "Flies", "Condors"])

# === Main Tab ===
with main_tab:
    st.subheader("Click a date on the rate time series or use arrow buttons")

    fig_ts_click = go.Figure()
    for col in maturities:
        if df_filtered[col].notna().sum() > 10:
            valid_data = df_filtered[['Date', col]].dropna()
            fig_ts_click.add_trace(go.Scatter(x=valid_data['Date'], y=valid_data[col], mode='lines', name=col))

    fig_ts_click.update_layout(title="Click on a date to show the SR3 curve", hovermode='x unified')
    ...
    x_numeric = list(range(1, len(maturities_available) + 1))
    fig_yc = go.Figure()
    fig_yc.add_trace(go.Scatter(x=x_numeric, y=yc, mode='lines+markers', line=dict(color='black')))
    fig_yc.update_layout(
        title=f"SR3 Curve on {row['DateOnly']}",
        xaxis=dict(
            tickmode='array',
            tickvals=x_numeric,
            ticktext=maturities_available,
            title="Maturity"
        ),
...
# === Outright Tab ===
with outright_tab:
    st.subheader("Outright Rates")
    maturity = st.selectbox("Select SR3 Contract", maturities, key="outright_select")
